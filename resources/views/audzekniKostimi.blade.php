<!--
Šis HTML dokuments nodrošina tērpu pārskatu un izsniegšanas funkcionalitāti baletskolas sistēmā.
Dokuments ietver navigācijas joslas ar saitēm uz dažādām lapām un tabulu, kurā ir redzami lietotāji, to grupas un iespēja piešķirt tērpus.
...
<html lang="lv">
  ...
  <body> @if(session('success')) @if(session('success') == true) <script>
      ...
      alert(":(");
    </script> @endif @endif @if (session('LomaID') == 0)
    <!--Navigācija-->
    ...
    <nav class="navbar navbar-expand-lg navbar-light navbar-lighter">
      ...
      <table class="table">
        ...
        <tbody> @foreach($LietotajsData as $ld) <tr>
            ...
            <td>
              <a class="btn btn-primary" href="/dataInsertAudzekniKostimi{{$KostimiID}}/{{$ld->personasKods}}">Izsniegt</a>
            </td>
          </tr> @endforeach </tbody>
      </table>
    </div>
